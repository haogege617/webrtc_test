...
#define  NN 320
	...
	void *aecmInst = NULL;
	FILE *fp_far  = fopen("speakerH.pcm", "rb");
	FILE *fp_near = fopen("micinH.pcm", "rb");
	FILE *fp_out  = fopen("outH.pcm", "wb");
	...
	do 
	{
		...
		WebRtcAec_Init(aecmInst, 32000, 32000);
		...
		int  filter_state1[6], filter_state12[6];
		int  filter_state2[6], filter_state22[6];
		int  Synthesis_state1[6], Synthesis_state12[6];

		memset(filter_state1, 0, sizeof(filter_state1));
		memset(filter_state12, 0, sizeof(filter_state12));
		memset(filter_state2, 0, sizeof(filter_state2));
		memset(filter_state22, 0, sizeof(filter_state22));
		memset(Synthesis_state1, 0, sizeof(Synthesis_state1));
		memset(Synthesis_state12, 0, sizeof(Synthesis_state12));
		while(1)
		{
			...
			{				
				...
				short shInL[160], shInH[160];
				short shOutL[160] = { 0 }, shOutH[160] = { 0 };
				memcpy(shBufferIn, far_frame, 320 * sizeof(short));

				WebRtcSpl_AnalysisQMF(shBufferIn, shInL, shInH, filter_state2, filter_state22);
				
				WebRtcAec_BufferFarend(aecmInst, shInL, NN/2);//对参考声音(回声)的处理

				fread(near_frame, sizeof(short), NN, fp_near);
				memcpy(shBufferIn, near_frame, 320 * sizeof(short));
				//首先需要使用滤波函数将音频数据分高低频，以高频和低频的方式传入降噪函数内部
				WebRtcSpl_AnalysisQMF(shBufferIn, shInL, shInH, filter_state1, filter_state12);

				WebRtcAec_Process(aecmInst, shInL, shInH, shOutL, shOutH, NN/2,40,0);//回声消除
				short shBufferOut[320];
				//如果降噪成功，则根据降噪后高频和低频数据传入滤波接口，然后用将返回的数据写入文件
				WebRtcSpl_SynthesisQMF(shOutL, shOutH, shBufferOut, Synthesis_state1, Synthesis_state12);
				memcpy(out_frame, shBufferOut, 320 * sizeof(short));
